  ...
  return (
    <button
      ...
      style={{
        ...
        "--star-light-width": `${lightWidth}px`,
        "--star-light-color": lightColor,
        "--star-border-width": `${borderWidth}px`,
        "--star-background": backgroundColor,
        isolation: "isolate",
      } as CSSProperties}
      className={cn(
        "star-button relative z-[3] overflow-hidden h-10 px-4 py-2 inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-3xl text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 group/star-button",
        className,
      )}
    >
      <div
        className="absolute aspect-square inset-0 animate-star-btn bg-[radial-gradient(ellipse_at_center,var(--light-color),transparent,transparent)] star-button-light"
        style={{
          offsetPath: "var(--path)",
          offsetDistance: "0%",
      ...
      />
      <div
        className="absolute inset-0 dark:border-white/15 border-black/10 z-[4] overflow-hidden rounded-[inherit] dark:text-black text-white star-button-stars"
        style={{ borderWidth: "var(--border-width)" }}
        aria-hidden="true"
      >
        <StarBackground color={backgroundColor} />
      </div>
      <span className="z-10 relative bg-gradient-to-t dark:from-white dark:to-neutral-500 from-black to-neutral-400 inline-block text-transparent bg-clip-text star-button-label">
        {children}
      </span>
    </button>
  );
